...
target_ID1  06 +05 0C 00 xx xx xx xx bin(0000 0001) xx [xx]*20
target_ID2  06 +05 0C 00 xx xx xx xx bin(0000 0010) xx [xx]*20
target_ID3  06 +05 0C 00 xx xx xx xx bin(0000 0011) xx [xx]*20
target_ID4  06 +05 0C 00 xx xx xx xx bin(0000 0100) xx [xx]*20
target_IDA  06 +05 0C 00 xx xx xx xx bin(0000 0101) xx [xx]*20

at_ID1  06 +05 0C 00  xx xx xx xx xx bin(0000 0001) [xx]*20
at_ID2  06 +05 0C 00  xx xx xx xx xx bin(0000 0010) [xx]*20
at_ID3  06 +05 0C 00  xx xx xx xx xx bin(0000 0111) [xx]*20
at_ID4  06 +05 0C 00  xx xx xx xx xx bin(0000 0100) [xx]*20
at_IDA  06 +05 0C 00  xx xx xx xx xx bin(0000 0101) [xx]*20

urgent_stop(abnormal reusme)  06 +05 0C  00 xx xx xx xx 08 xx.[xx]*20
buffer_stop(abnormal reusme)  06 +05 0C  00 xx xx xx xx 0B.xx.[xx]*20
...
05 05 00 00 00 05 00 00 00 00 00 00 00 0A 00    setIDA
...
05 05 00 00 00 05 00 00 00 00 00 00 00 0A 00    saveIDA
...
def callback_cmd_label(data):
    ...
    if label=="STATUS":
        ...
        c = [05, 05, 00, 00, 00, 0x01, 00, 00, 00, 00, 00, 00, 00, 0x06, 00]
    elif label=="setID2":
        c = [05, 05, 00, 00, 00, 0x02, 00, 00, 00, 00, 00, 00, 00, 0x07, 00]
    elif label=="setID3":
        c = [05, 05, 00, 00, 00, 0x03, 00, 00, 00, 00, 00, 00, 00, 0x08, 00]
    elif label=="setID4":
        c = [05, 05, 00, 00, 00, 0x04, 00, 00, 00, 00, 00, 00, 00, 0x09, 00]
    elif label=="setIDA":
        c = [05, 05, 00, 00, 00, 0x05, 00, 00, 00, 00, 00, 00, 00, 0x0A, 00]
    elif label=="saveID1":
        c = [05, 05, 00, 00, 00, 0x01, 00, 00, 00, 00, 00, 00, 00, 0x06, 00]
    elif label=="saveID2":
        c = [05, 05, 00, 00, 00, 0x02, 00, 00, 00, 00, 00, 00, 00, 0x07, 00]
...
def status_parse(input):
    if input[0]==0x0c:
        ...
        s = {}
        s['volt'] = '%0.1f' % (data[14] + data[15]*256)/10
        if data[5] == 0x08:
            s['stop_mode'] = 'urgent'
        elif data[5] == 0x0B:
            s['stop_mode'] = 'buffer'
