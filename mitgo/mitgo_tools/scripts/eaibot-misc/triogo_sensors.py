...
update (Oct 2017):
    limit range of get_blob_angle when odom_drift out of limit
...
class navSensors():
    ...
    # ags location
    scan= {'back':[],'front':[],'hud_F':[], 'hud_B':[], 'missctn':[0,20]}    ###pid
    ...
    def __init__(self, heading):
        ...
        self.interface = rospy.Publisher('/interface',String,queue_size=2)    #pid
    ...
    def get_params(self):
        ...
        self.semi_trig = rospy.get_param("~semi_trig", 0.6)
        self.align_blob = rospy.get_param("~align_blob", 0.20)
        self.align_acute = rospy.get_param("~align_acute", 1.2) # 0.25 1.2
        self.align_drift = rospy.get_param("~align_drift", 0.4) # 1.2 0.25 0.4
        ...
        self.goal_dist = rospy.get_param("~goal_dist", [0, 0.6 , 2.6, 1.9, 2.9, 7.0])
        ...
        self.ZBA_autoback = rospy.get_param("~ZBA_autoback", 10) # 10, 0 enable
    ...
    def auto_steering(self, error, ags_status, scan_object):
        ...
        driftz = self.ags_drift * copysign(self.align_drift, error)    ###pid
        acutez = self.pid_update(self.ags_acute * 2.0, driftz * 0.7)    ###pid (1, 0.7)
        spinz = acutez / self.base_length * self.align_acute    ###pid

        #spinz = self.ags_drift / self.base_length * copysign(self.align_drift, error)
        #spinz+= self.ags_acute / self.base_length * self.align_acute        
        ...
        move_factor *= 0.75 if abs(self.ags_drift) > 24 else 1.0    # > 20
    ...
    def auto_tracking(self, error, ags_status, scan_object, onhold):
        ...
        logimu = lambda imu: np.sin(imu*0.07) * 10 / self.base_length    ###pid
        sigimu = lambda imu: np.sin(imu*0.09) * 10 / self.base_length    ###pid   
        
        if ags_status=="on_track":
            driftz = self.ags_drift * copysign(self.align_drift, error)    ###pid
            acutez = self.pid_update(self.ags_acute, driftz * 0.5)    ###pid
            spinz = acutez / self.base_length * self.align_acute    ###pid
        else:
            spinz = 0
    ...
    ###pid
    def pid_reset(self, P=1.2, I=3, D=0.004):
        self.pid = PID.PID(P, I, D)
        self.pid.setSampleTime(0.004)
        self.value_pid = 0.0
        self.pid_update(0.0, 0.0)

    ###pid
    def pid_update(self, acutez, driftz):
        self.pid.SetPoint = acutez
        self.pid.update(self.value_pid - driftz)
        self.value_pid += self.pid.output
        self.value_pid = round(self.value_pid,1)
        
        return self.value_pid
    ...
    def callback_adc_lbat(self, adc):
        ...
        if adc0_batmon!= 0:
            if adc0_batmon < self.lbat_trig:
                self.low_batt = True
        self.voltage_value = adc0_batmon    ###pid       
    ...
    def callback_msg_scan(self, msg):
        ...
        self.scan['front']= ranges[-50:] + ranges[:150]    #-85:85
        self.scan['back']= ranges[-770:-570]              #-805:-635
        self.scan['hud_F']= ranges[-30:] + ranges[:170]  #-120:120
        self.scan['hud_B']= ranges[-790:-550]             #-840:-600
        self.scan['missctn'][0] = 0     ###pid
    ...
    def get_blob_angle(self):
        ...
        if isready:
            ...
            off_angle += -pi if off_angle > pi/2 else 0
            if abs(self.odom_drift)>0.18:
                if off_angle>0.1:    off_angle = 0.1
                if off_angle<-0.1:    off_angle = -0.1
    ...
    def get_ags_status(self):
        # correct sign of imu for drift and acute
        ...
        if (carrier_A1>0) and (carrier_B1>0):
            ...
            if imu_A1 is not None and imu_B1 is not None:
                self.ags_drift= imu_A1 - imu_B1    ###pid
                self.ags_acute= imu_A1 + imu_B1	   ###pid
    ...
    def get_scan_block(self, normdir=1):
        ...
        try:
            ...
            scan_focus = str(hudscan)
            
            self.ags_scan.publish(scan_focus)    ###pid
    ...
    def get_scan_object(self, normdir=1):
        ...
        try:
            self.scan['missctn'][0] += 1
            if self.scan['missctn'][0] > self.scan['missctn'][1]:
                return "impact"
         
            key= 'back' if normdir < 0 else 'front'
            rpscan= np.array(self.scan[key])
            scan_focus = str(rpscan)
            
            self.ags_scan.publish(scan_focus)    ###pid
 
            key= 'hud_B' if normdir < 0 else 'hud_F'  
            hudscan = np.array(self.scan[key])
