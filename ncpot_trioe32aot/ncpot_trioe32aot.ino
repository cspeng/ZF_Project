    ...
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      if (rxValue.length() > 0) {
        ...
        // set param
          ...
          if (rxNum>0) {
            setPoint = rxNum +T_const2 + varT_offset;
          ...
          int rxNum = String(rxValue.substr(6, 2).c_str()).toInt();          
          if (rxNum!=0) {
            varT_offset = rxNum;
...
void data_Convert_char()
{
   ...
   paraDataPak.chunnamed_1 = "-3";
   paraDataPak.chunnamed_2 = "-3";
   paraData = "$BLE,460,460,-3,10,20,333,333";
}
...
//    else { regBank.set(40006, 0); }
      ...
      mb.Hreg(2,(setPoint-T_const2)*10);
