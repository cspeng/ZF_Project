...
set branch_instruction:
05 05 00 00 01 00 00 00 00 00 00 00 00 06 00
"""
...
def callback_cmd_label(data):
    ...
    elif label == "branch_instruction":
        c = [05, 05, 00, 00, 0x01, 00, 0x00, 00, 00, 00, 00, 00, 00, 0x06, 00]
    else:
        c = []
...
def status_parse(input):
    ...
    if sync==0x0c:
        ...
        if data[4] == 0x08:
            ...
            s['stop_mode'] = 'off_track'
        elif data[4] == 0x20:
            s['stop_mode'] = 'frontAngleAbnormal'
        elif data[4] == 0x22:
            s['stop_mode'] = 'behindAngleAbnormal'
        elif data[4] == 0x0E:
            s['stop_mode'] = 'batteryExtremeLow'
        elif data[4] == 0x10:
            s['stop_mode'] = 'batteryLow'    
        else:
            s['stop_mode'] = ''
        ...
        s['instruction'] = 'turnrg' if(data[2] & 0b100!=0) else 'turnlf'
...
def base_publish(pub, info):
    ...
    msg.data = info['instruction']
    pub['instruction'].publish(msg)
    msg.data = info['stamp']
...
def comm_listern():
    ...
    pub['instruction'] = rospy.Publisher('instruction', String, queue_size = 2)
