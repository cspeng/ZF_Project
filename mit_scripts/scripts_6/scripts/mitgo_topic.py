...
class navTopic():
    ...
    status.update({'low_batt':False, 'is_forward':True, 'is_turnlf':True, 'is_hold': False, 'on_move':False})
    status.update({'stop_mode':'', 'volt':'', 'heading':'','instruction':'', 'stamp':'', 'target':'', 'at':''})
    status.update({'read_tag':'', 'rate_tag':'', 'branch_tag':'','align_hud':'', 'dispMsg':"      "})
    ...
    def __init__(self):
        ...
        rospy.Subscriber("instruction", String, self.callback_status_instruction)
    ...
    def callback_status_instruction(self, msg):
        self.status['instruction'] = msg.data
        self.status['is_turnlf'] = (msg.data == 'turnlf')
    ...
    def callback_reader(self, msg):
        ...
        if msg.frame_id[0]=='M':
            ...
            self.status['branch_tag'] = ''
            self.status['read_tag'] = msg.frame_id
        elif msg.frame_id[0] == 'B':
            self.status['branch_tag'] = msg.frame_id
        else:
    ...
    def callback_msg_scan1(self, msg):
        ...
        self.scan['hud_F']= ranges[515:975]    # 165deg == 660 
    ...
    def callback_msg_scan2(self, msg):
        ...
        self.scan['hud_B']= ranges[575:915]    # 165deg == 660 
    ...
    def is_turn_tag(self):
        return (self.status['branch_tag'] in self.branch_id)
    ...
    def get_scan_object(self, normdir=1):
        ...
        try:
            ...
            if np.any((rpscan < 0.55) * rpmask) + np.any(hudmask) + np.any(sidmask):
                return "block"
            elif np.any((rpscan < 0.90) * rpmask):    #1.3
                return "impact"
            elif scandog > 20:
                return "noscan"
