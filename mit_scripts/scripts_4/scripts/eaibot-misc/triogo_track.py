...
created by clarence PEMS@2017
    ...
    o/s:
        ...
        reverse move off track at station AA ### disable move_orient 
...
class NavGO():
    ...
    def ags_status(self):
        navi = self.navi
        self._status= navi.get_ags_status()
        self.interface_info(self._status)

        ags_info = " %s %s  " % (self.zone, self.gear)

        ags_info += "%s, %s" % (navi.ags_acute, navi.value_pid) ###pid
        ags_info += "/ %s  " % (navi.ags_drift)
        ags_info += navi.tagrf['frame']
        ags_info += " on_tag  " if navi.is_rftag_lastN() else "  "
        ags_info += "%s  %sdeg " % (navi.align_hud, navi.get_blob_angle())
        ags_info += "drift %s " % (navi.odom_drift)
