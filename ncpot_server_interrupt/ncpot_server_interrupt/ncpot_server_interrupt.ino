...
void setup() {
  ...
  pinMode(7, INPUT_PULLUP);    // button switch
  pinMode(8, OUTPUT);             // button indicator
  pinMode(6, OUTPUT);             // buzzer horn
  pinMode(5, OUTPUT);             // not-used as if +V for horn
  digitalWrite(6, HIGH);             // horn-off
  digitalWrite(5, HIGH);             // +5V
  digitalWrite(8, HIGH);             //lighting-off
  attachInterrupt(digitalPinToInterrupt(7),keyhandle,RISING);
...
void keyhandle(void)
{
  ...
  if(millis() - lastDebounceTime > debounceDelay){
   ...
   if(interruptTime == 2){
     if(btnPress == true){
      btnPress = false;
      lastCountTime = lastDebounceTime;
      if(!I8){I8 = true;}
      digitalWrite(8, LOW);
   }
   }
