...
void setup() {
  ...
  potID = mb_potid();
  if (!ModbusRTUServer.begin(potID, 19200)) {
...
void loop() {
  ...
  key_time = ModbusRTUServer.holdingRegisterRead(2);
  ...
  do {
    ...
    char remain_str[9];
    String remain_sec = count_down() + " time";
...
void button_states(void) {
      ...
      if ((millis() - lastDebounceTime) > debounceDelay) {
          ...
          if (reading==LOW & btnPress) {
                ...
                if (!I8) {I8 = true; key_time++; ModbusRTUServer.holdingRegisterWrite(2, key_time);}
                btnPress = false;  
           }
      }
