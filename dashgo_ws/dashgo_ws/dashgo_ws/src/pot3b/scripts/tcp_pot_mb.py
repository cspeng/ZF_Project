        ...
        while True:
            try:
                ...
                if self.r.get("clear_count") == "True":
                    ...
                    self.master.execute(self.addr, cst.WRITE_SINGLE_REGISTER, 8, output_value = 2)
                    time.sleep(0.1)
                    self.master.execute(self.addr, cst.WRITE_SINGLE_REGISTER, 9, output_value = 2)
                    time.sleep(0.1)
                    self.r.set("clear_count",False)
                   
                if "_set_param_" in self.r.get("write_param"):
					params = self.r.get("write_param")
					self.pot_set = int(params[12:15])
					self.PID_action(self.pot_set + 1)
					
					time_set1,time_set2 = (40, 40)
					try:
