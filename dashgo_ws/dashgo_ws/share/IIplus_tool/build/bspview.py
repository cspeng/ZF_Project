...
'''
import sys, atexit
from tkForm2 import winApp
import pypyodbc, time
import pi_modbus

pi_modbus._tool_offset= [0.0, -0.05, -0.07, -0.03, 0.0]
...
class sqlConn():
    ...
    def sqlDB(self):
        ...
        fetch= self.sqlExpr(uid= '08149242')    #08149640
        ...
        if fetch is not None:
            ...
            if len(fetch):
                for select in fetch:
                    ...
                    table.append(select[0:-2]+(self.Electric_state[flag],))
    ...
    def sqlToolEnquiry(self,selvals):
        fetch= None
        if self.isConn():
            try:            
                if self.isTMC:
    ...
    def test(self, DS,ecid,s=None):
        SQL1= "select * from TMICS.tmicsuser.ElectricScrewdriver_NoticePEProofStrength "
        SQL2= "update TMICS.tmicsuser.ElectricScrewdriver_NoticePEProofStrength "        
        pars= [s,ecid]
        SQL1+= "where Id=?"
        SQL2+= "set State= ?   where Id=?"
        if s is None:
            pars=(ecid, '5.76', 'Kgf.cm', u'合格', '01160262', '2015-08-29', '08149242', '')
            (ecid,median,unit,RET,tool,lt,cardid,REM)=pars
            resp= DS.sqlToolUpdate(ecid,median,unit,RET,tool,lt,cardid,REM)
        else:
            self.cur.execute(SQL2,pars)
            self.cur.commit()
            resp= None
        #countercheck
        pars= [ecid]
