...
#mainFrame
class winApp(Frame):
    """App frame and form V2.0"""       # XXX ok
    ...
    BOX = ["None","2.5 Nm","10  Nm","20  Nm","search"] 
    ...
    def transferE2(self):
        ...
        # XXX transfer input E2BUSY and output E2e2
    ...
    def onTimeout(self):
        ...
        if self.rCH is not None:
            ...
            #return next loop
            if self.rCH.get('mbNew')!='True':
                self._job= self.root.after(self.DELAY, self.onTimeout)
...
class toolTree(object):
    """treeview with jobno icons"""         # XXX ok
...
class _PopupNumKey(Toplevel):
    '''A Toplevel instance that displays a keyboard'''      # XXX ok    
...
def PopupNumKey(parent, attach, keysize=4, keycolor='gray'):
    """ Numerical keyboard Popup """    # XXX ok
...
class winDialog(tkdialog):
    ...
    def body(self, master):
        ...
        self.cb = Checkbutton(master, text="Forward rotation",state='disable')
        self.cb.select()
...
class ui_widgets(object):
    """ UI interface prototype """      # XXX ok
    def __init__(self,root,fetch,DS=None):
        #init user field
        ui= {'key':StringVar(), 'kcol':'JobNo', 'kix': 1,
             'alt':StringVar(), 'acol':StringVar(), 'aix': 3,
             'ulab':StringVar(), 'user':None, 'opts':None, 
             'uid':None, 'wait':None, 'allbtn':None,'fbtn':None}
        self.ui_style(root)
        ...
        fbtn= Button(text= "過濾列表",font=("Calibri", 14), command= self.onFilter)
        ...
        if not self.isRowDialog:
            try:
                ...
                if selvals!='':
                    ...
                    fetch=self.DS.sqlToolEnquiry(selvals)
