...
class NavGO():
    ...
    def __init__(self, navi):
        ...
        rospy.Subscriber('batmon',Adc,self.callback_Adc)
    ...
    def callback_Adc(self, adc):    ###
        WAKE_data = ""
        navi = self.navi
        adc0_batmon = adc.adc0
        pathway_data = self._status
        scanbc = str(navi.align_hud)
        if dispMsg.data == "WAKElb":
		    WAKE_data = "WAKElb"
        elif pathway_data[0:3] == "off":
		    WAKE_data = "WAKEof"
        elif scanbc == "block":
            WAKE_data ="WAKEbc"
        else:
		    WAKE_data ="______"
        InterfaceData = "$AGV,"+str(adc0_batmon) + "," + dispMsg.data + "," + WAKE_data + "\r\n"
