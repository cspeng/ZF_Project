...
class BaseController:
    def __init__(self, base_frame):
        ...
        pid_params = dict()
        pid_params['wheel_diameter'] = rospy.get_param("~wheel_diameter", 0.1280) 
        pid_params['wheel_track'] = rospy.get_param("~wheel_track", 0.3559)
        pid_params['encoder_resolution'] = rospy.get_param("~encoder_resolution", 1200) 
        pid_params['gear_reduction'] = rospy.get_param("~gear_reduction", 1.0)
        pid_params['Kp'] = rospy.get_param("~Kp", 50)
        pid_params['Kd'] = rospy.get_param("~Kd", 20)
    ...
    def cmdVelCallback(self, req):
        ...
        self.v_des_left = int(left * self.ticks_per_meter / 30)
        self.v_des_right = int(right * self.ticks_per_meter / 30)


class OdomROS():
    def __init__(self):
		rospy.init_node('OdomSIM', log_level=rospy.DEBUG)
		...
		rospy.loginfo("use Odom SIM instead of Arduino Driver...")
		...
		if self.use_base_controller:
			self.myBaseController = BaseController(self.base_frame)
    ...
    def shutdown(self):
        ...
        try:
            ...
            pass
        rospy.loginfo("Shutting down OdomSIM Node...")
