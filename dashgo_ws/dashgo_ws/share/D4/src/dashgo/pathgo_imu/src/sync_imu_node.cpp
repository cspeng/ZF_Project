...
int main(int argc, char** argv)
{
  ...
  uint8_t size=8;  // revised 15 to 8
    ...
    try
    {
      if (ser.isOpen())
      {
        if(ser.available())
        {
          ...
          position = input_hex.find("FFFF"); //revised AA00 to FFFF

          if (position != input_hex.npos) 
          {
              if(position!=0){
                  position = input_hex.rfind("FFFF");
                  std::string input_1 = ser.read((0u+position/2)); 
                  input=input_0+input_1;
                  //ROS_WARN_STREAM("IMU unsync hex_str: " << 0u+position/2 << " " << string_to_hex(input));
              }else{
                  input=input_0;
              }
              
              char *chr = &input[0u+position/2];
              short angle = ((chr[4] & 0xFF) | ((chr[5] << 8) & 0XFF00));
              short angleRate = ((chr[2] & 0xFF) | ((chr[3] << 8) & 0XFF00));
              short check_sum = 0xFFFF + angle + angleRate;
              
              bool not_valid = ((unsigned char)(check_sum & 0xFF) != chr[6]);
              not_valid = not_valid || ((unsigned char)((check_sum>>8) & 0xFF) != chr[7]);

              if (not_valid) 
              {
                std::string head_hex = string_to_hex(input.substr(0u+position/2));
                ser.read((1));
                ROS_WARN_STREAM("IMU invalid head_str: " << head_hex);
              }
              else
              {
                  current_time = ros::Time::now();
                  ...
                  // start values of orientation covariance
              ...
              std::string input_1 = ser.read((7)); 
              input=input_0+input_1;
              ROS_WARN_STREAM("IMU miss hex_str: " << string_to_hex(input));
          } 
        }
      }
      else
      {
        ...
        if(ser.isOpen())
        {
            ...
            free(cmd_packet);
            ROS_INFO_STREAM("IMU Serial port " << ser.getPort() << " initialized.");
            ser.read((120)); // heading string 0D0A...0D0A
        }
        else
        {
