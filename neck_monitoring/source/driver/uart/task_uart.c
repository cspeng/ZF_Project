...
void TaskUART_taskInit(void)
{
  ...
  UART_read(UARTHandle, Uart_RxTempBuf, UART_BUF_LEN);       //打开一个串口读
}
...
{ 
  ...
  while(1)
  {
    ...
    {
      UART_read(UARTHandle, Uart_RxTempBuf, UART_BUF_LEN);     //再次打开一个串口读
    }
...
{ 
  ...
  //UART_read(handle, Uart_RxTempBuf, UART_BUF_LEN);    //再次打开一个串口读
