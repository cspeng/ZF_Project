...
class winApp(Frame):
    ...
    BOX = ["None","12 Nm","2  Nm","20  Nm","search"] 
    ...
    def start(self):
        ...
        self._job= self.after(self.DELAY, self.onTimeout)
        print "app starts now."		###checkpoint
    ...
    def initFrame(self):
        ...
        self.root.wm_title("III Inone on cloud")
    ...
    def otherMenu(self,menubar):
        ...
        holdMenu.invoke(holdMenu.index(self.HOLD[1]))   
        menubar.add_cascade(label="PEAK-Hold", menu=holdMenu)
        #box submenu
        boxMenu = Menu(menubar, tearoff=0)  
        for i in (1,):  #(1,2,3,4)
            boxMenu.add_radiobutton(label=self.BOX[i],variable=self.bx, value=i)
        boxMenu.invoke(boxMenu.index(self.BOX[1]))   
...
#XXX preparing 
    ...
    def mbBXkey(self):
        # Single Box handler #
        if self.rCH.get('mbNew')!='True':
            alt_stat = ['None', 'Park', 'None', 'None', 'None']
    ...
    def transferE2(self):
        ...
        #print("busy %s  E2ack %s  E2e2 %s" % (E2BUSY, self.E2ACK, self.rCH.get('E2e2')))
...
class tkdialog(Toplevel):
    """ interface dialog with fetch """
    # {'winfo_xy':(x,y), 'title':title, 'message':message, 'fgcolor':color}
    def __init__(self, parent, fetch, rCH = None, tool = None, remark=None):
        ...
        self.geometry('%dx%d+%d+%d' % (450, 400, x, y))
...
class toolDialog(tkdialog):
    ...
    def init_values(self):
        # set tryfail in this.afterTimeout to False when in development!
        self.step= 0
        self.retctn= 20*1.2
    ...
    def afterTimeout(self):
        # using transferE2() run in main dataShow
        ...
        if E2e2 == '1' and self.step < 3:       #log count
            ...
            #set tryfail to True not in development state
            if self.step==0:    self.tryfail = False    #True
            if E2FLAG=="FAIL" and self.tryfail:
                self.tryfail = False
            else:
                self.step+= 1
                E2text= "  %1.3f        %s\n" % (E2PEAK,E2FLAG)
...
class ui_widgets(object):
    ...
    def ui_style(self,root):
        ...
        style.configure('.', font=('lucida', 11))
...
class uiFrame(ui_widgets):
        ...
        try:
            ...
            if selvals!='':
                ...
                if vkey!=1:
                    self.isRowDialog= False
                    self.ui['key'].set("")  
                    print "cancel measurement as out of range"
                    return
    ...
    def verifybox(self, spec, spec_convert):
        ...
        if upper_limit > 12.0:
            vkey = -1
            print "spec higher than torque limit 12m!"
        elif upper_limit < 0.1:
            vkey = -1        
            print "spec lower than torque limit 0.1m!"    
        else:
            vkey = 1
