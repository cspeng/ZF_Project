...
_tool_scale= (0.0, 6.0, 2.5, 20.0 , 0.0)
_tool_round= (1.0, 5.0, 5.0, 2.0, 1.0)
_tool_offset= [0.0, -0.07, -0.02, -0.03, 0.0]
#_tool_offset= [0.0, -0.07, -0.07, -0.03, 0.0]
_tool_lowcut = [0.1, 0.1, 0.03, 0.2, 0.1]
thread= None
...
cdef int convloop(float ADM, float ADP, int ctn, r):
    ...
    #get mode and offseted abs peak value
    mode= r.get('modeAD')
    xabs= abs(ADP)
    lowcut_value = _tool_lowcut[int(r.get('mbBX'))]

    #update peak value and reset auto counter
    if  xabs > abs(_PEAK) and xabs >= lowcut_value:
        ctx.AUTOZ_CTN= 0
        if _SIGN==2:
            E2BUSY= 1
            E2PEAK= round(xabs,3)
        ...
        if abs(_VAL+_OFF) < lowcut_value:   #historical 0.025, 0.05
            ctx.AUTOZ_CTN += 1
    ...
    def AD_callback(self):
        ...
        try:
            ...
            if (ADx[0] & 0x08):	
                ...
                ctx.conv_CTN= 1 
                if ctx.RQOFF==0:     
                    _OFF= _tool_offset[int(r.get('mbBX'))]
                    ctx.RQOFF= int(-_OFF * 30000.0 / scale)  
