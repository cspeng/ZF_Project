    ...
    def mainloop_thread(self):
        ...
        while not self.is_shutdown():
            ...
            try:
                if data is not None:
                    last_peak = ds._PEAK
                    ds._PEAK = int(data[16:21])
                    ds._VAL = [int(s) for s in data[21:].split(",")][1]
                    ...
                    if ds._PEAK > last_peak and last_peak>50:    ctx.E2_notOK= 1
                    if ctx.AUTOZ_CTN > 6 and mode=='AUTO PEAK':    ctx.E2_notOK= 0

                    #keep stable on auto mode
                    if val_os > 100 or ds._PEAK > last_peak:
                        ctx.AUTOZ_CTN = 0
